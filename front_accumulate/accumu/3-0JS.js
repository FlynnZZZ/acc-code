介绍 
  JS诞生于1995年,最初目的用于验证表单输入;
  作用:用来实现网页上的特效效果,动画或交互的实现;
  解释型的,基于对象和事件驱动并具有相对安全性的客户端脚本语言「scripting-language」
  JS的复杂特性API「类似于网络、存储和图形等」是由其 宿主环境「host environment」提供;
专业术语 
  脚本语言：不具备开发操作系统的能力,只用来编写控制其他大型应用程序的“脚本”
  实例： 类型 的具象化;在面向对象的编程中,通常把通过类创建对象的过程称为实例化
  函数和方法的区别
    方法就是对象的函数
    方法基于对象,调用写法：obj.foo()
    函数基于过程,写法：foo()
