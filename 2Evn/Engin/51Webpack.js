...
'entry'入口: 作为构建其内部依赖图的开始  
  PS: 默认: './dist'   
  ◆配置 
  ,context: path.resolve(__dirname, 'src')    // 上下文,入口文件所处的目录的绝对路径 
  ...
  ,entry: {                  // 适用于多入口,最可扩展的方式 
    ...
    // key 映射到 [name] 中
    key1: './src/index.js'
    ,key2: ['./entry1.js','entry2.js'] // 将被打包到一起 
    ,vendor: ['vue','vue-router','vuex'] // 将第三方库进行单独打包 
      // 一般使用'vendor',也可以是其他任意字符串 
  }
'output'出口: 指定打包后的文件的输出位置  
  ◆配置 
  ,output: {    
    path: path.resolve(__dirname, 'dist')    // 指定输出目录,需用绝对路径   
    ,filename: './bundle.js' // 输出bundle的名称  
      PS: 也可包含路径,会接着path后 
      使用占位符输出多文件 
        PS: [hash]和[chunkhash]等占位符长度可以使用[hash:16]来指定,默认:20
        [name]      模块名称,对应 entry.xxx  
        [id]        模块标识符,对应chunk的id  
        [hash]	    模块标识符的 hash
    ...
    ,chunkLoadTimeout: num // chunk请求到期前的毫秒数,单位:ms '2.6.0+' 
      // 默认:120000 
    ...
    ,auxiliaryComment: str/obj  
    ,crossOriginLoading: bol/str 
    ,jsonpScriptType: KW 
      "text/javascript"   默认值, 
      "module"            使用ES6模式  
    ,devtoolFallbackModuleFilenameTemplate: str 
    ,devtoolModuleFilenameTemplate: str 
    ,devtoolNamespace: str 
    ,devtoolLineToLine: bol/obj  
    ,hashDigest: KW    // 生成hash时使用的编码方式   
      PS: 支持 Node.js hash.digest 的所有编码 
      'hex'  默认值 
    ,hashDigestLength: num  // 散列摘要的前缀长度,默认:20 
    ,hashFunction: KW   // 散列算法 
      PS: 支持 Node.JS crypto.createHash 的所有功能 
      'md5'   默认值 
    ,hashSalt: str   // 可选的加盐值 
    ,hotUpdateChunkFilename: str  // 自定义热更新 chunk 的文件名 
      PS: 占位符只能是 [id] 和 [hash] 
      "[id].[hash].hot-update.js"   默认值 
    ,hotUpdateFunction: fn  // 用于加载热更新的 JSONP 函数
  }
