...
界面 
  wx.setNavigationBarTitle({ // 动态设置当前页面标题 
    title: str // 必选,页面标题文字 
    ,success: function(back){ } // 可选,调用成功的回调 
    ,fail: function(info){ }     // 可选,调用失败的回调 
    ,complete: function(result){ } // 可选,调用结束的回调 
  })
  ...
  交互反馈 
    ...
    wx.showModal({  // ​显示模态弹窗 
      ...
      ,cancelColor: "#ccc" // 取消按钮的文字颜色,默认:"#000"
...
网络 
  ...
  wx.uploadFile({   // 上传文件,将本地资源上传到服务器,返回 uploadTask  
    ...
    ,success: function(back){ // 接口调用成功的回调函数 
      back
        .data       开发者服务器返回的数据
        .statusCode HTTP状态码
    }
...
位置 
  wx.getLocation({    // 获取当前地理位置信息 
    success: function(back){  // 必选,成功的回调 
      back 
    ...
    ★低功耗蓝牙接口 
            ...
            back
              .characteristics  array  设备特征值列表
                characteristic对象 蓝牙设备characteristic(特征值)信息
                  ...
                  .properties.read     boolean  该特征值是否支持 read 操作
                  .properties.write    boolean  该特征值是否支持 write 操作
                  .properties.notify   boolean  该特征值是否支持 notify 操作
                  .properties.indicate boolean  该特征值是否支持 indicate 操作
              .errMsg  string  成功:ok,错误:详细信息
          },
          fail: function(){ },
          complete: function(){ },
      ...
      wx.readBLECharacteristicValue({}) 读取低功耗蓝牙设备的特征值的二进制数据值 '1.1.0'
          ...
          read接口读取到的信息需要在onBLECharacteristicValueChange方法注册的回调中获取。
        {
          ...
          success: function(back){
            back
              .characteristic obj,设备特征值信息.蓝牙设备characteristic(特征值)信息
                .characteristicId  str,蓝牙设备特征值的 uuid
                .serviceId         obj.蓝牙设备特征值对应服务的 uuid
                .value      ArrayBuffer,蓝牙设备特征值对应的二进制值 
                  注意:vConsole 无法打印出 ArrayBuffer 类型数据 
              .errMsg         str,成功:ok,错误:详细信息 
          },
          fail: function(){ },
          complete: function(){ },
        }
