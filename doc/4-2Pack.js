...
'webpack.config.js'默认的配置文件: 通过该文件进行打包配置 
  ...
  module.exports = {  // commonjs模块化输出 
    ...
    'entry': { // 适用于多入口的情况,最可扩展的方式  
      ...
      key3: ['vue','vue-router','vue-resource','vuex'], // 将第三方库进行单独打包 
    },
    'entry': ['./entry1.js','entry2.js'], // 将多个文件打包在一起
    'entry': './src/index.js', // 指定单一的入口文件 
    // 入口文件: 将被打包的文件 
    'output': {  // 指定打包后的文件的输出 
      path: path.resolve(__dirname,'dist'),  // 指定输出目录,对应一个绝对路径   
      // 打包后的文件的名称 
    ...
    'devServer': {}, // 'webpack-dev-server'配置 [详见插件]
    ...
    'module': {    
      ...
      rules: [    // '2.x'版本写法 
        { test: regp, // 可选,匹配特定条件,正则表达式或正则表达式的数组  
          ...
          use: "aoo-loader", // 使用单个loader,简写方式
        ...
        { test: /\.js$/,
          ...
          options: {
            presets: ['latest',"es2015","stage-0"], // 指定将会编译的版本 
  ...
  url-loader    在JS中处理引入的图片文件 
    { test: /\.(png|jpe?g|gif|svg)(\?.*)?$/,
      use: [
        { loader: 'url-loader',
          options: {
            limit: 10000,     // 限制图片大小 10000B，否则转换为 base64格式
  ...
  'webpack-dev-server'    静态资源服务器  
    ...
    'webpack.config.js'中的配置 
      'devServer': { 
        ...
        progress: true, // 
        hot: true,  // 启用HMR'Hot Module Replacement模块热替换 
