概念
  ...
  NoSQL 
    PS:指的是非关系型的数据库,数据存储不需要固定的模式,无需多余操作就可以横向扩展
...
MongoDB 
  基于分布式文件存储的数据库,由C++编写,旨在为WEB应用提供可扩展的高性能数据存储方案 
  介于关系数据库和非关系数据库之间,是非关系数据库当中功能最丰富,最像关系数据库的 
'document'文档 : MongoDB的一条记录 
  一个包含多个字段的数据结构,类似于JSON格式  
'collection'集合 : 文档储存的地方
  类似于关系型数据库的表,在一个集合之中,记录的格式并不需要相同 
  每个集合之中的每个文档,必须有一个'_id'字段作为主键 
