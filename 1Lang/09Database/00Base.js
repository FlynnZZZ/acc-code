...
RDMBS'Relational DataBase Management System'关系数据库管理系统 
  ...
  相关概念/术语[自我总结版本]: 
    数据库: 表的集合  
      数据库的名字应该是唯一的 
    表table: 记录的集合,由一系列的行和列组成 
    行/记录: 一条数据,将表拆分为若干结构相同的部分 
    列/字段: 表或记录的一部分,将表拆分为若干相对独立的部分 
    NULL 值: 表中以空白形式出现的值,表示该记录在此字段处没有设值 
      NULL 值同 0 值或者包含空格的字段是不同的
      值为 NULL 的字段是在记录创建的时候就被留空的字段 
    SQL 约束: 是表中的数据列必须遵守的规则,用于限制表中数据的类型 
      ...
      主键：唯一标识数据表中的行/记录,必须包含唯一值,并且不能为 NULL 
      外键：唯一标识其他表中的一条行/记录,用于关联两个表 
        外键为单一字段或者多个字段的组合,并与另外一个数据表的主键相匹配
      索引：用于在数据库中快速创建或检索数据,使用索引可快速访问数据库表中的特定信息  
