...
介绍_概念_说明_定义
  ...
  引入
    下载并用 <script> 标签引入，Vue 会被注册为一个全局变量。
    e.g. :
    <script src="http://cdn.jsdelivr.net/vue/1.0.7/vue.min.js" charset="utf-8"></script>
  特点
    声明式渲染
...
v-xx 指令Directives 
  PS:带特殊前缀的 HTML 特性，可以让 Vue.js 对一个 DOM 元素做各种处理。
  ...
  v-show : 作用与v-if类似
    不同的是v-show的元素会始终渲染并保持在 DOM 中,且v-show不支持<template>标签
  v-text 渲染数据
  v-html
  v-attr
  v-repeat
...
组件系统 
  PS:组件系统是 Vue.js 另一个重要概念,提供了一种抽象,用独立可复用的小组件来构建大型应用。
    如果我们考虑到这点，几乎任意类型的应用的界面都可以抽象为一个组件树.
    在一个大型应用中，为了使得开发过程可控，有必要将应用整体分割成一个个的组件.
    Vue.js 组件非常类似于自定义元素.
    Vue.js 组件提供了原生自定义元素所不具备的一些重要功能，
    比如组件间的数据流，自定义事件系统，以及动态的、带特效的组件替换。
--------------------------------------------------------------------------------
★React 
介绍_概念_说明_定义
  起源于Facebook,
  因公司对市场上所有JavaScript MVC 框架都不满意而写的一套JS库,用来架设Instagram网站 
