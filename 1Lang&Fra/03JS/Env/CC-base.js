...
Scene场景编辑器 用来展示和编辑场景中可视内容的工作区域 
...
Node_Tree层级管理器:用树状列表形式展示场景中的所有节点和他们的层级关系 
...
Properties属性检查器:查看并编辑当前选中节点的组件属性的工作区域 
  ...
  Button 按钮组件 
    ...
    Transition  用于设置当按钮按下时的状态变化
      ...
      Duration 过渡时间
      Zoom_Scale 缩放比例[可为负]
    Click_Events 点击事件,列表类型,默认为空 
      PS:为方便设计师在制作UI界面时可以自行指定按钮功能而设置;
        数组类型,通过更改其数量,添加指定数量的响应函数;
        目前只支持 Click 事件,即当用户点击并释放 Button 时才会触发相应的回调函数
      Target    [带有脚本组件的]节点
      Component [target上的]脚本组件
      Handler   [component内的]回调函数,点击Button时触发
      CustomEventData 指定任意字符串,作为事件回调的最后一个参数传入
  ProgressBar 进度条
...
--------------------------------------------------------------------------------
JS脚本 
  PS:在脚本中 this 表示该脚本组件对象或脚本组件的properties对象[SlPt];
◆cc 模块 
  PS:Cocos引擎的主要命名空间,引擎代码中所有的类,函数,属性和常量都在这个命名空间中定义
★子模块 
  cc._decorator 模块
  cc.js 模块
★类型 
  cc.Node      节点类 
    ...
    ccNode.parent  读写,节点的父节点
      node.parent = parentNode   更改节点的父节点
      同parentNode.addChild(this.node)方法等价;
      注意,通过 创建和销毁节点 介绍的方法创建出新节点后,
      要为节点设置一个父节点才能正确完成节点的初始化。
    parentNode.addChild(ccNode)  添加子节点
    node.children      以数组形式返回所有子节点 
    node.childrenCount 返回子节点数量
    ccNode.getChildren()  获得一个包含所有子节点的数组
      // CannonManager.js
      cc.Class({
        extends: cc.Component,
        start: function () {
...
★方法 
  节点相关 
    ...
    cc.isValid(ccNode)  判断当前节点是否已被销毁
  组件相关 
  位置尺寸 
    cc.p(x,y /obj)  返回Vec2对象 
      x   optional,Number,default=0
      y   optional,Number,default=0 
      obj 如 {x: 100, y: 100}
  动作变化 
    cc.moveBy(duration,deltaPos[,deltaY])  移动指定的距离,返回ActionInterval
  ...
  其他方法 
    cc.log();   打印日志
    cc.error(); 抛出错误
    cc.callFunc(selector[selectorTarget=null][data=null]) 执行回调函数,返回ActionInstant
      selector Function
...
★属性对象 
  cc.director  管理游戏逻辑流程的单例对象,创建和处理主窗口并管理执行场景 
    ...
    ◆方法
    loadScene('sceneName'[,cfoo]) 通过场景名称进行加载场景 
      cfoo  指定场景加载后的回调函数
    ...
    preloadScene('sceneName'[,cfoo]) 预加载场景,可以在任何时候调用该方法 
      PS:调用完后,仍需通过 loadScene 来启动场景,因为该方法不会执行场景加载操作; 
        就算预加载还没完成,也可直接调用 cc.director.loadScene,加载完后场景就会启动;
  ...
  cc.game 
    cc.game.addPersistRootNode(myNode) 常驻节点
      PS:通常同时只会加载运行一个场景,当切换场景时默认会将场景内所有节点和其他实例销毁,
...
◆归纳总结[待归类] 
    ...
    properties 属性声明 
      ...
      type    类型声明
        ...
        Example:
          ...
          enemy: {
            default: null,
            type: cc.Node   // 表示一个 Node 控件
          }
        
          将 type 设置为 cc.Integer,表示整数[这样在'属性检查器'中就不能输入小数点]
          score: {
            default: 0,
            type: cc.Integer
          }
      ...
      visible 布尔值,是否在'属性检查器'中显示 
        PS:默认情况下,若属性名以下划线 _ 开头,则不在'属性检查器'中显示,
          若设置 visible 为 true,则仍会显示;
        properties: {
          _aoo : {      // 下划线开头默认会隐藏
            default: 0,
            visible: true  // 设置为 true 则会在属性检查器中显示
          }
          boo : {       // 非下划线开头默认会显示
            default: 0,
            visible: false // 设置为 false 则会在属性检查器中隐藏
          }
        }
      serializable 布尔值,指定了 default 默认值的属性默认情况下都会被序列化,
      ...
      serializable 布尔值,序列化该属性,设置false就是临时变量 
        详见 serializable 参数
        不能用于 get 方法
      editorOnly   默认false,在导出项目前剔除该属性
        不能用于 get 方法
  ...
  Node 节点类 
    PS:修改变换的方法会影响到节点上挂载的渲染组件,比如 Sprite 图片的尺寸、旋转等等;
      使用nod来表示节点实例,方便后续描述
    ▼获取节点
    this.node [在组件中]表示组件所在的节点 
      start : function () {
        var node = this.node;
        node.x = 100;
      }
    ▼操作节点
    node.active = false 该节点和其后代节点上的组件都将被禁用
    node.active = true  该节点和其后代节点都将被激活[除非其后代节点单独设置过关闭]
    node.position  读写 position 变量
      node.position = cc.p(0, 0)   设置 position 变量
    ccNode.setPosition(params)   设置 position 变量
      params  可为 0, 0 或 cc.p(0, 0) 
    node.x = 100;  更改节点x轴坐标
    node.y = 50;   更改节点y轴坐标
    node.rotation = 90; 更改节点旋转
    node.setRotation(90); 更改节点旋转
    ccNode.destroy() 销毁节点
      PS:销毁节点并不会立刻发生,而是在当前帧逻辑更新结束后,统一执行;
        当一节点销毁后,该节点就处于无效状态,可通过 cc.isValid 判断;
    ...
    node.scaleX = 2; 更改节点x方向缩放
    node.scaleY = 2; 更改节点y方向缩放
    node.setScale(2, 2); 节点缩放
      传入单个参数时,会同时修改 scaleX 和 scaleY。
    node.setContentSize(100, 100);  更改节点尺寸
    node.setContentSize(cc.p(100, 100)); 更改节点尺寸
    node.width = 100;  更改节点尺寸
    node.height = 100; 更改节点尺寸
    node.anchorX = 1; 更改节点锚点位置
    node.anchorY = 0; 更改节点锚点位置
    node.setAnchorPoint(1, 0); 更改节点锚点位置
    node.color = cc.Color.RED; 设置颜色
    node.opacity = 128; 设置不透明度 
      在使用 Sprite, Label 这些基本的渲染组件时,
      修改颜色和不透明度的操作只能在节点的实例上进行,
      因为这些渲染组件本身并没有设置颜色和不透明度的接口。
    nod.runAction(action) 执行并返回该执行的动作,该节点将会变成动作的目标,返回Action 
      PS:调用 runAction 时,节点自身处于不激活状态将不会有任何效果。
    ...
    ▼节点与组件
    ccNode.addComponent(cmptType) 在节点上添加组件,返回添加的组件
  ...
  Component 组件类 
    PS:使用cmpnt来表示组件实例,方便后续描述;
    cmpnt.getComponent()    获得同一个节点上的其它组件 
      通过组件类型获取 
        start : function () {
          var label = this.getComponent(cc.Label);
    ...
    ccLabel.string     读写label组件的文本 
      ccLabel.string = 'abc';
    cmpnt.node        该组件所在的节点 
    cmpnt.schedule(cfoo,interval,times,time)  计时器
      cfoo     func,
    ...
    cmpnt.scheduleOnce(cfoo); 一次性的计时器
      var mySch =function(){ 
        console.log("schedule Once log..."); 
      }
      this.scheduleOnce(mySch);
    cmpnt.unschedule(cfoo);   取消定时器
    ★Sprite 精灵组件 
      ★SpriteFrame 精灵的精灵帧
        PS:使用 spfm 表示实例,便于后续描述;
        spfm.setTexture(textureOrTextureFile[rect=null][rotated=false][offset=cc.v2(0,0)][originalSize=rect.size])  通过 Texture,rect,rotated,offset 和 originalSize 设置 SpriteFrame,返回Boolean
  ...
  ActionInterval类 时间间隔动作,这种动作在已定时间内完成,继承FiniteTimeAction 
    ...
    getDuration()  获取动作以秒为单位的持续时间,返回Number 
      继承自 FiniteTimeAction:
    setDuration(duration) 设置动作以秒为单位的持续时间。
      继承自 FiniteTimeAction:
      duration Number 
    reverse()  返回一个新的动作,执行与原动作完全相反的动作,返回Null 
      继承自 FiniteTimeAction:
    clone()  返回一个克隆的动作,返回FiniteTimeAction 
      继承自 FiniteTimeAction:
    isDone()  如果动作已完成就返回 true,返回Boolean 
      继承自 Action:
    getTarget() 获取当前目标节点,返回Node 
      继承自 Action:
    setTarget(target) 设置目标节点 
      继承自 Action:
      target Node 
    getOriginalTarget()  获取原始目标节点,返回Node 
      继承自 Action:
    getTag()  Number 获取用于识别动作的标签,返回Number 
      继承自 Action:
    setTag(tag)  设置标签,用于识别动作 
      继承自 Action:
      tag Number
  访问已有变量里的值 
    若已经在一个地方保存了节点或组件的引用,也可以直接访问它们;
    window.xx 通过全局变量访问
  ...
  加载和切换场景 
    PS:在 Cocos Creator 中,使用场景文件名[可不包含扩展名]来索引指代场景。
    cc.director.loadScene('MyScene', cfoo); 通过场景名称进行加载场景 
    cc.director.preloadScene      预加载场景
    使用预加载场景资源配合 runScene 的方式进行预加载场景的方法已被废除 
      // 请不要再使用下面的方法预加载场景!
  ...
  事件 
    ★生命周期回调 
      PS:Cocos Creator 为组件脚本提供了生命周期的回调函数。
        只要指定回调函数,Creator 就会在特定的时期自动执行相关 脚本,不需要手动调用。
      onLoad    回调会在该组件所在的场景被载入时触发 
        PS:在组件脚本的初始化阶段,保证可获取到场景中的其他节点,以及节点关联的资源数据,
          通常会在 onLoad 阶段去做一些初始化相关的操作
        cc.Class({
          extends: cc.Component,
          properties: {
      ...
      start     回调函数会在组件第一次激活前 
        PS:也就是第一次执行 update 之前触发, start 通常用 初始化一些中间状态的数据,
      ...
      update    通常用于在每一帧渲染前更新物体的行为,状态和方位
        cc.Class({
          extends: cc.Component,
      ...
      lateUpdate  在所有组件的 update 都执行完之后才进行操作
        update 会在所有动画更新前执行; lateUpdate 用于在动画更新后进行一些额外操作
        cc.Class({
          extends: cc.Component,
      ...
      onEnable   当组件的 enabled 属性从 false 变为 true 时触发
        倘若节点第一次被 创建且 enabled 为 true,则会在 onLoad 之后,start 之前被调用。
      onDisable  当组件的 enabled 属性从 true 变为 false 时触发 
      onDestroy  当组件调用了 destroy(),会在该帧结束被统一回收,从而触发回调
    ★事件绑定 
      PS:除了通过button组件绑定事件的方式外,可选方式如下
      node.clickEvents.push(EventHandler) 绑定事件 
        首先构造 EventHandler 对象,
        然后设置对应的 target component handler 和 customEventData 参数,
        最后通过 node.clickEvents.push(EventHandler) 进行事件的绑定;
        和使用编辑器添加的事件回调相同; 
        //here is your component file, file name = MyComponent.js 
        cc.Class({
          extends: cc.Component,
          ...
          onLoad: function () {
            ...
            clickEventHandler.target = this.node; // 为脚本组件所属的节点
            clickEventHandler.component = "MyComponent";//为脚本文件名
      ...
      button.node.on('click', ...)  事件名注册 
        一种简便的方式,但有一定的局限性,在事件回调里面无法 获得当前点击按钮的屏幕坐标点
        //假设我们在一个组件的 onLoad 方法里面添加事件处理回调,在 callback 函数中进行事件处理:
        cc.Class({
          extends: cc.Component,
          properties: {
          ...
          callback: function (event) {
            // event 为一 EventCustom 对象
            var button = event.detail; // 获取 Button 组件
      ...
      nod.once('eventName',cfoo)   一次性的事件监听 once
        this.node.once("touchend",function (event){
          console.log("touchend event"); 
        });
      nod.off("eventName",cfoo,this);  关闭监听
      nod.emit("eventName",obj)  手动触发事件[发射事件]
  ...
  模块访问 
    ...
    // require + 文件名 [不含路径] 来获取到其他 模块 的对象
    var aoo = require("test");
    aoo.boo = 991;
    console.log(aoo.boo); // 991 
--------------------------------------------------------------------------------
构建打包发布  
--------------------------------------------------------------------------------
示例代码 
