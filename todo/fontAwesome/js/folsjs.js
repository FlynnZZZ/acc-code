...
var ecmf =function(){ }; 
  // 字符串操作
  // 根据下标替换字符串中的字符
  // 将str中的index下标的字符替换为s,返回值为替换后的新字符串
  ecmf.prototype.repla =function(str,s,index1){
    var res =str;
    // var ind =arguments.slice(2);
    // ind表示输入的所有下标的数组
    // 实际证明arguments不可切片,只好循环遍历复制
    var ind =[]
    ...
    for (var i = 0; i < ind.length; i++) {
      res =res.slice(0,ind[i])+s+res.slice(ind[i]+1);
    }
    return res;
  }

  // 数组操作
    // 返回数组中条件元素的下标 func为条件判断函数
  ecmf.prototype.seeindex =function(arr,func){
    var len =arr.length
    for (var i = 0; i < len; i++) {
      if (func(arr[i])) {
        console.log("得到的符合条件的下标",i)
        return i
      }
    }
    console.log("没有执行到")
